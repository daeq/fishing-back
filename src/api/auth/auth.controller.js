...
exports.localRegister = async ctx => {
  ...
  ctx.body = { ...account.profile, email: account.email }; // 프로필 정보로 응답합니다.
};
...
exports.changeAccountClassification = async ctx => {
  const { userInfoId, accountClassificationId } = ctx.request.body;
  const user = await Account.findOneAndUpdate(
    { _id: userInfoId },
    {
      $set: { 'profile.accountClassificationId': accountClassificationId }
    },
    { upsert: true }
  );
  const accountClassification = await AccountClassification.findOne({
    _id: accountClassificationId
  });
